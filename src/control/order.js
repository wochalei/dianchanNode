...
class Order {
    ...
    async get(ctx) {
      ...
      const sql = `db.collection("order").get()`
      const res = await DB.link('get', sql);
      new formatResponseData(ctx, res.data.data).sendBody();
    }
    async add(ctx) {
      const { userId, name, } = ctx.request.body;
      const getSql = `db.collection("order").where({name:"${name}"}).get()`;
      const addSql = `db.collection("order").add({
            data:[
                {
                    userId:"${userId}", 
                    name:"${name}",
    ...
    async updated(ctx) {
      ...
      const sql = `db.collection("order").doc("${id}").update({data:{
        name:"${name}",
        }})`;
    ...
    async deleted(ctx) {
      ...
      const sql = `db.collection("order").doc("${id}").remove()`;
